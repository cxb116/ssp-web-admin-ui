...
import { getSspSlotHourTrend } from '#/api/data/sspslothour';

import { useGridFormSchema } from '../data';

defineOptions({ name: 'DataSspSlotDayChartReport' });

const props = defineProps<{
  /** 页面检索条件（mediaId/appId/sspSlotId/dspSlotId/dspSlotCode/osType 等） */
...
/** 小时可选值（与 sspslothour 一致） */
const hourOptions = [
  { label: '全天', value: -1 },
  ...Array.from({ length: 24 }, (_, i) => ({ label: `${i}`, value: i })),
];

/** 小时报表的时间检索字段（与 sspslothour 一致） */
...
async function buildTrendParams(): Promise<Record<string, any>> {
  ...
  if (formValues.sspSlotId) {
    const s = String(formValues.sspSlotId).trim();
    params.sspSlotId = s
  ...
  if (reportType.value === 'day') {
    ...
    // 与 sspslothour 一致：8位=全天，10位=指定小时
...
async function loadChart() {
  ...
  if (reportType.value === 'day') {
    ...
    const data = await getSspSlotHourTrend(params);
    rows.push(...(data as any[]));
...
async function getJumpQuery(): Promise<Record<string, any>> {
  ...
  const fields = ['mediaId', 'appId', 'sspName', 'sspSlotId', 'osType'];
...
async function handleDayReport() {
  if (props.context === 'hour') {
    router.push({ name: 'DataSspSlotDay', query: await getJumpQuery() });
  } else {
    emit('back');
  }
}
...
async function handleHourReport() {
  if (props.context === 'hour') {
    ...
    router.push({ name: 'DataSspSlotHour', query: await getJumpQuery() });
  }
}
